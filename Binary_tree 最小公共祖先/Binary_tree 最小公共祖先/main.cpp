...
public:
    ...
    Binary_node<Entry>* com_ancestor(Binary_node<Entry>* sub_root,Binary_node<Entry>* p,Binary_node<Entry>* q);
    Binary_node<Entry>* Ancestor(Binary_node<Entry>* p,Binary_node<Entry>* q);
    Binary_node<Entry>* search_node(const Entry &target)const;
protected:
...
template<class Entry>
Binary_node<Entry>* Binary_tree<Entry>::Ancestor(Binary_node<Entry>* p,Binary_node<Entry>* q){
    return com_ancestor(root, p, q);
}

template<class Entry>
Binary_node<Entry>* Binary_tree<Entry>::com_ancestor(Binary_node<Entry>* sub_root,Binary_node<Entry>* p,Binary_node<Entry>* q){
    if(sub_root == NULL || sub_root->data==p->data || sub_root->data== q->data)
        return sub_root;
    Binary_node<Entry>* lleft = com_ancestor(sub_root->left, p, q);
    Binary_node<Entry>* rright = com_ancestor(sub_root->right, p, q);
    if(lleft == NULL && rright == NULL) return NULL;
    else if(lleft != NULL && rright != NULL) return sub_root;
    else if(lleft != NULL) return lleft;
    return rright;
}
...
template <class Entry>
Error_code Binary_tree<Entry>::tree_search( Entry &target){
...
template<class Entry>
Binary_node<Entry>* Binary_tree<Entry>::search_node(const Entry &target)const{
...
int main()
{
    ...
    for(int i = 0;i < num; i++){
        int read;
        cin>>read;
        Mytree.BST_insert(read);
    }
    int boy,girl;
    cin>>boy>>girl;
    Binary_node<int>* BOY = Mytree.search_node(boy);
    Binary_node<int>* GIRL = Mytree.search_node(girl);
    Binary_node<int>* ANS = Mytree.Ancestor(BOY, GIRL);
    cout<<ANS->data <<endl;
    return 0;
}
